...
class SpriteAnimator {
     ...
     */
    getTotalFrames() {
        // 計算總幀數：每行幀數乘以總行數（總行數通過總高度除以每幀高度向上取整得到）
    ...
    draw(ctx, originalX, originalY) {
        // 只有當圖片載入完成後才進行繪製
        if (this.image.complete) {
            const totalFrames = this.getTotalFrames(); // 獲取總幀數
            // 確保總幀數和動畫速度有效，避免除以零
            if (totalFrames > 0 && this.animationSpeed > 0) {
                // 根據總播放時間和動畫速度計算當前幀的索引
                // Math.floor(this.currentTime / this.animationSpeed) 計算出已經過去了多少個完整幀的時間間隔
                // % totalFrames 確保幀索引在 0 到 totalFrames - 1 之間循環，實現動畫的重複播放
                const frameIndex = Math.floor(this.currentTime / this.animationSpeed) % totalFrames;
